...
A Time-Triggered Network is a time-triggered system deployed on the Time-Triggered Network-on-Chip (TTNoC) implementation. 
...
At present, time-triggered systems are widely deployed in various field.
TTEthernet integrates time-triggered concept into the general Ethernet~\cite{DBLP:conf/nca/SteinerBHPV09}, which is adopted as a new standard of the avionic network~\cite{DBLP:conf/dsrt/BejiHGM14}. 
TTP and FlexRay are adopted in aerospace industry~\cite{DBLP:journals/taes/HuLWV15}.
...
Given two scheduling of messages $s_i,s_j\in\calS$, %$s_{i},s_{j}\in\mathcal{M}, s_{i}\neq s_{j}$,
...
The MA requires genetic representation,
 fitness function,
  global search such as crossover, mutate,
   and local search. 
...
According to the genetic representation, we formally give the algorithm. 

The population $pop$ is initialized by $initPop(scale)$.
The size of population $scale$ and the maximum iterations $N$ are predefined.
...
The algorithm is iterative executed until we derive a feasible scheduling or an upperbound of the iteration number $N$ is reached.
...
Next, the operation of $crossover(pop)$ and $mutate(pop)$ are employed as global search strategies.
...
And the individual with least score in $pop$ is selected as the best one $best'$ by function $selectBest(pop)$.
The historical optimal individual $best$ is compared with current optimal individual $best'$,
...
Since the score represents the conflict times, the smaller of the score the better of the individual.
