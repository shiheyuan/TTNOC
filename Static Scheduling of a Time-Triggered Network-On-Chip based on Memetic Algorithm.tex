...
\author{\IEEEauthorblockN{Heyuan Shi\IEEEauthorrefmark{1},
		Jiaxiang Liu\IEEEauthorrefmark{2}}
	\IEEEauthorblockA{
		School of Software\\
		Tsinghua University,
		Beijing, China 30332--0250\\ Email: hey.shi@foxmail.com}
}
...
The duration of communication $m_{i}$ in its $\mathit{k}$th period is defined by $s_i(k)$, which equals
\begin{equation}
	s_i(k)
	=
	[
	...
	%{m_{i}^{(v_{a},v_{b})}}.\mathcal{T}+{m_{i}^{(v_{a},v_{b})}}.\phi+{m_{i}^{(v_{a},v_{b})}}.\mathcal{L}			
	]
\end{equation}

Therefore for any 
$	s_i,s_j \in \calS, s_i \neq s_j	$,
%$ {m_{i}^{(v_{a},v_{b})}}\in\mathcal{M}, {m_{j}^{(v_{c},v_{d})}}\in\mathcal{M}$,
we have that
\begin{equation}
	s_i.R \cap s_j.R \neq \emptyset
	\Longrightarrow
	s_i(p) \cap s_j(p) = \emptyset
\end{equation}
%\begin{equation}
%	(v_{a},v_{b})
%	\cap
...
% $\mathcal{M} = \{m_{1},\dots,m_{i}\}$ for each $\tau_{i}\in \Gamma $ to satisfy the set of given scheduling constrains. 
